...
Les graphiques sont l'un des plus puissants leviers d'explication des données. Il n'y a guère de meilleur moyen de parler des données et de les faire "parler" que de les donner à voir.
...
Pour manipuler les données, il faut les emmagasiner dans des variables, des conteneurs dont le contenu est appelé à changer au fil des opérations (d'où le nom "variable" attribuer aux conteneurs). Les structures de données varient selon le nombre de dimensions qu'elles comportent (1 à *n* dimensions) et le nombre de types qu'elles acceptent. Le vecteur et la matrice, par exemple, sont dits "atomiques", c'est-à-dire qu'on ne peut y mettre qu'un seul type de données. Le premier possède une seule dimension tandis que la seconde en a deux. Le tableau de données, très utilisé en science des données, est très polyvalent: il accepte des données de plusieurs types (un seul type par colonne, cependant) et possède deux dimensions.
...
Quelles sont les variables discrètes et continues dans le graphique de Minard? Pouvez-vous faire le lien avec la dimension esthétique du tableau, soit l'élément visuel chargé de représenter ces variables?
